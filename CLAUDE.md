...
A prototyping playground for Design client engagements. A designer (Mary) builds
...
- `packages/ui/` — `@design/ui`, the shared component kit. Token-driven, client-agnostic. **Protected.**
- `themes/` — global themes (`design`, `design-dark`, …), available to every client. **Protected.**
  Keep `themes/design.yaml` in sync with `designTokens` in `packages/ui/src/tokens.ts`.
...
3. Client folders never import app internals — only `@design/ui`.
...
- Icons come from `@design/ui/icons` (Lucide; browse/search at `/icons`) —
...
hooks — fix and recommit. PRs merge on green (enable GitHub auto-merge so green
merges itself); the boundary rules keep client work out of `app/` and
`packages/`, and changes to the core are deliberate.
