...
import { TEAM_HEADERS, call, db, seedSchema } from "./test-harness";
...
describe("listThreads", () => {
  it("returns only the requested client's threads", async () => {
    const res = await call("/api/threads?client=acme", { headers: TEAM_HEADERS });
  ...
  it("filters by status", async () => {
    const res = await call("/api/threads?client=acme&status=applied", { headers: TEAM_HEADERS });
    expect(ids(res.json.threads)).toEqual(["t2"]);
  });

  it("filters by route", async () => {
    const res = await call("/api/threads?client=acme&route=/home", { headers: TEAM_HEADERS });
