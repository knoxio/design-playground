...
import { CLIENT_HEADERS, HELIX_HEADERS, SERVICE_HEADERS, call, seedSchema } from "./test-harness";
...
describe("identify — helix domain user", () => {
  ...
  it("honors a configurable helix domain via HX_HELIX_DOMAIN", async () => {
    const headers = { "cf-access-authenticated-user-email": "lead@helix.example" };
    const create = await call("/api/threads?client=acme", {
      method: "POST",
      headers,
      helixDomain: "helix.example",
      body: { route: "/r", anchorKind: "css", anchor: ".x", body: "hi" },
    });
    const id = String(create.json.id);
    const status = await call(`/api/threads/${id}?client=acme`, {
...
describe("identify — non-helix client email", () => {
  it("authenticates but cannot moderate", async () => {
    ...
    expect(String(status.json.error)).toContain("Helix-only");
  });
});
