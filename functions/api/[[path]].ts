 ...
 * their own client by the HX_CLIENT env var; the internal surface passes
...
function identify(request: Request, env: Env): Identity | null {
  ...
  const domain = env.HX_HELIX_DOMAIN ?? "helixcollective.com";
  const isHelix = email !== null && email.toLowerCase().endsWith(`@${domain}`);
  const isService = email === null;
  return { email, canModerate: isHelix || isService };
}
...
async function listThreads(env: Env, client: string, url: URL): Promise<Response> {
  ...
  const threads = await env.HX_DB.prepare(query)
    .bind(...params)
    .all();
  const messages = await env.HX_DB.prepare(
...
async function addMessage(
  ...
  const thread = await env.HX_DB.prepare("SELECT id FROM threads WHERE id = ? AND client = ?")
    .bind(threadId, client)
    .first();
  if (!thread) return json({ error: "thread not found" }, 404);
  await env.HX_DB.prepare(
    "INSERT INTO messages (id, thread_id, author, body, created_at) VALUES (?, ?, ?, ?, ?)",
  )
...
async function setStatus(
  ...
  if (!who.canModerate) return json({ error: "status changes are Helix-only" }, 403);
  const status = str(body.status);
  if (status === null || !STATUSES.includes(status)) {
    return json({ error: `status must be one of ${STATUSES.join(", ")}` }, 400);
  }
  const result = await env.HX_DB.prepare(
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const client = env.HX_CLIENT ?? url.searchParams.get("client");
  if (client === null) return json({ error: "client query parameter is required" }, 400);

  await ensureSchema(env.HX_DB);
