...
description: Promote a client component into the shared kit (@helix/ui). Core-owner operation — its own reviewed PR, never bundled with client work. Use when a promotion candidate is approved for the kit.
---
...
- The kit component must look right under any theme (check at least the
  promoting client's theme + helix + helix-dark on the gallery).
