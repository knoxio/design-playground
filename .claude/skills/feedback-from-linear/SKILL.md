...
3. For each issue, create or update a thread on the feedback service for the
   client. Threads are created with `POST $DP_FEEDBACK_URL/threads?client=<id>`
   ...
   - **Dedup:** record the Linear issue id in the thread (in the first message,
     ...
     (`list_threads` via the `design-feedback` MCP) by that id — update, never
     duplicate.
