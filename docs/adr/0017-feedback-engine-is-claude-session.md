...
`open | applied | rejected | outdated`. The `hx-feedback` MCP server exposes
...
- The payload format, the skill, and the service carry over unchanged if
  Helix later pays for in-page editing.
...
exactly this casual-iteration workflow. Rejected; revisit only if Helix wants
