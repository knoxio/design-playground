...
- **Pages** — one project for the internal app, one `design-preview-<id>` project
...
- **Worker + D1** — the feedback API ships as Pages Functions inside every
  surface, all sharing one `design-feedback` D1 database. Same-origin `/api/*`,
...
- One D1 database serves every surface, scoped server-side by `DP_CLIENT`
