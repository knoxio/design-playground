...
`pnpm boundaries`, the scoped-build leak check, and CODEOWNERS):

1. **Clients-only PRs cannot touch `app/` or `packages/ui/`.** A change under
   `clients/**` merges on green without review; touching the core requires
   the core owner's review.
...
4. **Clients import only `@helix/ui`** (plus their own folder). The registry
