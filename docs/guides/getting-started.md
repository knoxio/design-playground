...
Onboarding for anyone picking up the Helix Playground — a designer returning to
...
- **Git** and a GitHub account with write access to `helix-collective/hx-playground`.
...
```sh
git clone git@github.com:helix-collective/hx-playground.git
cd hx-playground
pnpm install
...
server proxies `/api/*` to that deployed service, and the `hx-feedback` MCP
...
2. Fill in the three values in `.env` (it is gitignored — never commit it):
   ...
   - `HX_FEEDBACK_URL` — the deployed feedback API base (the default in
     `.env.example` is correct for the shared deploy)

   The service token is created in Cloudflare Zero Trust → Access → Service
   tokens. **The actual credential values live in Bitwarden** — get them from
   there (or from the team), do not generate ad-hoc ones. The values are never
   stored in this repo.

3. The `hx-feedback` MCP server is already registered in `.mcp.json`. It loads
...
The team's normal path is to open Claude Code in the repo and tell it to set
this up: it walks the `.env` copy, points you at Bitwarden for the values, runs
