...
Three areas are **protected** — the playground's shared foundation, changed
deliberately; the boundary rules are CI-enforced:

| Path           | What                                                                   |
| -------------- | ---------------------------------------------------------------------- |
| `app/`         | Playground core: shell, routing, registry, theming, overlay            |
| `packages/ui/` | `@design/ui`, the shared component kit (token-driven, client-agnostic) |
| `themes/`      | Global house themes (`design`, `design-dark`, …)                       |
...
3. Clients never import app internals — only `@design/ui`.
...
`@design/ui` is a real internal product with you as its curator. Budget the
...
- **Merging is squash-only.** PRs merge on green (enable GitHub auto-merge so
  green merges itself) — this is what lets Mary work without you in the loop.
  Treat changes to `app/`, `packages/`, `themes/`, or root config as
  deliberate: the boundary rules are CI-enforced.
...
- **Access** — auth. The internal app is open to any `@example.com`
