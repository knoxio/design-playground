...
> Provisioning a fresh Cloudflare account? Start with the
> [Cloudflare setup guide](../guides/cloudflare-setup.md) — this runbook
> describes the running system; that guide creates it from zero.
...
| -------------- | --------------------- | ------------------ | -------------------------------------------- |
| Internal app   | `design-playground`   | full (all clients) | inspect, themes, variants, comment, resolve  |
| Client preview | `design-preview-<id>` | `VITE_CLIENT=<id>` | one client: view + comment, prototype banner |
...
- **`@example.com` passes everywhere.** The `team-emails` policy
  (domain `var.team_email_domain`) is on the internal app and on every gated
  preview.
...
- **`public: true` is the only opt-out.** It removes the preview from
  ...
  deploys, but only team emails pass.
...
- The Access denied page shows `var.access_contact` ("ask your Design
  contact"), not a bare 403.

Sessions last 24h. The internal app's policy precedence is team-emails (1),
service-token (2).
...
team-domain users and service tokens can moderate (set thread status); client
...
`design-feedback` (`infra/feedback.tf`), bound as `DP_DB` into every Pages project
...
| PATCH  | `/threads/<id>`          | design / service | set status                    |

Previews are hard-scoped by the `DP_CLIENT` env var; the internal app passes
...
`scripts/feedback-mcp.mjs` (the `design-feedback` MCP server, `.mcp.json`) and the
...
`CF_ACCESS_CLIENT_SECRET`, `DP_FEEDBACK_URL` — see `.env.example`). This lets
...
| `variables.tf` | `account_id`, `team_email_domain`, `access_contact`             |
| `clients.tf`   | `local.clients` / `preview_projects` (from the clients dir)     |
| `pages.tf`     | internal + per-client Pages projects, the `DP_DB` binding       |
| `access.tf`    | the design policy, internal app, per-preview apps and policies  |
...
- **State lives in R2** (S3-compatible backend, bucket `design-playground`). The
...
- **Wrappers:** `pnpm infra:plan`, `pnpm infra:apply`. A human can run them
  without Claude.
- **Provider token** needs Cloudflare Pages: Edit, Access: Apps and Policies:
  Edit, and D1: Edit (the `design-feedback` database in `feedback.tf`). All
  account-scoped. See [cloudflare-setup.md](../guides/cloudflare-setup.md#2-api-token-deploy--terraform).
...
app/dist` to `design-playground` or `design-preview-<id>`. Runs when checks passed
...
— kept in your secrets manager, never committed, never in this doc. Copy
`.env.example` to `.env` and fill it from there (or mint one per
[cloudflare-setup.md](../guides/cloudflare-setup.md#4-access-service-token-local-feedback-overlay)).
