...
preview: # optional — client-facing preview deploy access (PRD-08)
  emails: # individual stakeholders (all @example.com always pass)
...
`preview` block means the preview exists but only team emails pass.
...
Global themes are the house set (`design`, `design-dark`, …) — shared surface,
...
```tsx
import type { PageMeta } from "@design/ui";
...
Client-specific components — wrappers and overrides built on `@design/ui` —
...
code imports `@design/ui` and its own client folder, nothing else. The YAML
