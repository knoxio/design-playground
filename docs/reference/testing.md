...
**The API layer** is security-relevant: it asserts each identity class (helix
...
| Command               | Runs                                                  |
| --------------------- | ----------------------------------------------------- |
| `pnpm test`           | layer 1A–1C (Vitest, `@helix/app`)                    |
...
env (`app/vitest.config.ts`); the aliases mirror `vite.config.ts` so `@helix/ui`
...
`HELIX_HEADERS`, `CLIENT_HEADERS`, `SERVICE_HEADERS` for the identity classes.
...
free. Play-tests are written against the neutral `PlayContext` from `@helix/ui`
