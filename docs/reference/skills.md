...
| `promote-component`     | Promote a client component into `@helix/ui` | —                   | [04](../prds/prd-04-prototyping-experiments.md) | 0003, 0004       |
...
- **Inputs:** `id` (kebab-case, unique), `name`, optional source client whose
  default theme is copied (default: global `themes/helix.yaml`).
...
- **Script:** `scripts/dtcg-to-theme.mjs` (`pnpm dtcg-to-theme <tokens.json>`)
  — DTCG document → theme YAML, missing groups falling back to the Helix
...
- **Inputs:** client; threads via the `hx-feedback` MCP server, a pasted
...
Promote a client component into the shared kit (`@helix/ui`).
...
- **Does:** reads the source for the flow's screens, order, data, and components;
  ...
  data, and client components built on `@helix/ui` — never copying real data or
...
- **Does:** reads issues via the Linear MCP, creates/updates one thread per issue
  (`POST $HX_FEEDBACK_URL/threads`), dedupes by recorded Linear issue id, lands a
