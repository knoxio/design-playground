...
The Design Playground is a Vite + React single-page app that wraps thin tooling
around Claude Code. A designer builds interactive client prototypes by talking
to Claude and interacting with the running prototype; Design engineers later
...
├── packages/ui/    @design/ui — the shared, token-driven component kit             (protected)
...
4. Clients import only `@design/ui`; `app/src/registry/` is the single
...
prototype canvas only; the playground chrome is Design-branded and never
...
`design-feedback` MCP server exposes the same threads to Claude sessions, and a
...
Access for any `@example.com` email. A client-facing preview is never
