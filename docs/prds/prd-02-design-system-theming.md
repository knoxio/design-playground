...
zod; the parsed-theme type lives in `@design/ui` alongside it:
...
`:root`. The chrome (shell, dock, overview) keeps the static Design defaults and
...
- **Kit manifest:** `@design/ui` exports `kitManifest`
...
(DTCG document → theme YAML, Design-standard fallbacks flagged with `# review:`
...
any omitted group falls back to the Design standard with a review warning — never
