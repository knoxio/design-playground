...
| ----------------------------------- | ---------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------- |
| **Internal app**                    | full (all clients)                 | any `@example.com` email (Cloudflare Access)               | everything: inspect, themes, variants, comment, resolve |
| **Client preview** (one per client) | `VITE_CLIENT=<id>` scoped (PRD-06) | per-client email list from `client.yaml` + all team emails | view + comment threads                                  |
...
- **Default-deny (ADR [0008](../adr/0008-access-based-confidentiality.md)):** no
  `preview` block → the preview deploys but only team emails pass Access.
...
1. Merge a new client with `preview.emails` → its preview URL is live; listed
   emails and team emails pass Access, others get the friendly denial.
