...
- Anchor precision degrades by surface:
  ...
  | Kind       | Source                                          | Where                    |
  | ---------- | ----------------------------------------------- | ------------------------ |
  | `source`   | `data-dp-source` file:line (compile-time stamp) | internal builds only     |
...
- Context per thread: client, route, theme key, viewport. The `[data-dp-ui]`
...
A compile-time Vite/Babel transform stamps DOM elements with `data-dp-source`
...
- Preview builds carry no source stamps (PRD-06 grep proves it) and no status
  ...
  moderation is Design-only, enforced server-side either way.
...
5. A preview build contains no `data-dp-source`; comments work behind Access with
