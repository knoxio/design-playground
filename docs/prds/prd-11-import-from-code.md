...
- **Writes**, into `clients/<id>/` only, a contract-valid result:
  - **Pages** in `pages/` — a single screen as a file, an ordered journey as a
    ...
    `@helix/ui` kit components, falling back to client `components/` wrappers
...
- Honors every boundary rule: no cross-client imports, no import of app
  internals, client code imports only `@helix/ui` and its own folder (ADR
...
4. The result imports only `@helix/ui` and the client's own folder — boundaries
   green.
