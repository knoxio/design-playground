 ...
 * back to the Design standard with a review warning — never silently invented.
 * This closes the "shipped but unvalidated" gap on the Figma import path.
 */

const fallback = parseYaml(readFileSync(new URL("../themes/design.yaml", import.meta.url), "utf8"));
...
const completeDoc = {
  ...
  font: {
    ...
    $extensions: { "co.design.numericVariant": "tabular" },
  },
  typeScale: fullScale,
...
describe("dtcgToTheme", () => {
  ...
  it("falls back to the Design standard with a review warning for a missing group", () => {
