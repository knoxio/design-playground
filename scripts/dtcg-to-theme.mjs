 ...
 * converts. Missing groups fall back to the Design standard with a
...
const DESIGN_STANDARD = new URL("../themes/design.yaml", import.meta.url).pathname;
 ...
 * `fallback` (the Design standard) for any group the document omits. Returns
...
export function dtcgToTheme(doc, fallback) {
  ...
  const fallbackFor = (name) => {
    warnings.push(`${name}: not in the DTCG document — Design standard used`);
  ...
  const theme = {
    ...
    type: {
      ...
      numbers: doc.font?.$extensions?.["co.design.numericVariant"] ?? "proportional",
...
function main() {
  ...
  const fallback = parseYaml(readFileSync(DESIGN_STANDARD, "utf8"));
