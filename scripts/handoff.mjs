...
function dtcgShadow(value) {
  ...
  if (layers.some((l) => l === null)) {
    return { $type: "shadow", $value: [], $extensions: { "co.design.css": value } };
  }
...
function toDtcg(tokens) {
  return {
    ...
    font: {
      ...
      $extensions: { "co.design.numericVariant": tokens.type.numbers },
    },
...
function collectKitUsage(files) {
  ...
  for (const source of sources) {
    for (const match of source.matchAll(/import\s*\{([^}]+)\}\s*from\s*"@design\/ui"/g)) {
