...
const helixTheme = readFileSync(path.join(repoRoot, "themes/helix.yaml"), "utf8");

describe("clientScaffold", () => {
  it("produces the contract's file tree", () => {
    const files = clientScaffold({ id: "acme", name: "Acme Co", themeYaml: helixTheme });
  ...
  it("rejects a non-kebab-case id and an empty name", () => {
    expect(() => clientScaffold({ id: "Acme", name: "x", themeYaml: helixTheme })).toThrow();
    expect(() => clientScaffold({ id: "acme", name: "", themeYaml: helixTheme })).toThrow();
  });
});

describe("writeClient", () => {
  it("writes the tree to disk and refuses to overwrite", () => {
    const root = mkdtempSync(path.join(tmpdir(), "hx-new-client-"));
    // Seed a minimal repo state the script reads (the Helix standard theme).
    writeClient(root, { id: "acme", name: "Acme Co" }, helixTheme);
    const dir = path.join(root, "clients", "acme");
    expect(readdirSync(dir).toSorted()).toContain("client.yaml");
    expect(() => writeClient(root, { id: "acme", name: "Acme Co" }, helixTheme)).toThrow(
      /already exists/,
    );
  });
});
