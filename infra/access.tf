# Default-deny access: every surface sits behind Cloudflare Access. Helix
...
resource "cloudflare_zero_trust_access_policy" "helix" {
  account_id = var.account_id
  name       = "helix-emails"
  decision   = "allow"

  include = [{
    email_domain = { domain = var.helix_email_domain }
  }]
}

resource "cloudflare_zero_trust_access_application" "internal_app" {
  account_id          = var.account_id
  name                = "hx-playground (internal)"
  ...
  policies = [
    {
      id         = cloudflare_zero_trust_access_policy.helix.id
      precedence = 1
    },
    {
