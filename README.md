...
A prototyping playground for Design client engagements. A designer builds
...
| `packages/ui/`   | `@design/ui` — the shared component kit, token-driven             | yes       |
| `themes/`        | Global themes available to every client (`design`, `design-dark`) | yes       |
...
"Protected" paths are the playground's shared foundation — change them
deliberately; the boundary rules below are CI-enforced. Client work lives in
`clients/**` and merges on green.
...
- **Themes** — YAML token sets (colors, type, density, radii, shadows),
  ...
  CSS variables on the prototype canvas only; the chrome stays Design-branded
...
through the `design-feedback` MCP server (or a pasted "Copy for Claude"
...
`design-feedback` MCP server (registered in `.mcp.json`; it loads when a
...
- **Merging:** squash-only; PRs merge on green (enable GitHub auto-merge so
  green merges itself).
...
- `docs/guides/` — getting started, for designers, for developers, Cloudflare setup
