    ...
    visitor: {
      JSXOpeningElement(nodePath, state) {
        ...
        const already = node.attributes.some(
          (a) => a.type === "JSXAttribute" && a.name.name === "data-dp-source",
        );
        if (already) return;
        const rel = path.relative(repoRoot, state.filename);
        node.attributes.push(
          t.jsxAttribute(
            t.jsxIdentifier("data-dp-source"),
...
/**
 * Stamps host elements in client pages with `data-dp-source="<file>:<line>"`
