import { spacingForDensity, type TokenSet } from "@helix/ui";
...
function SpacingSection({ tokens }: { tokens: TokenSet }) {
  ...
  return (
    <Section title="Spacing">
      <p data-hx-token="density" className="mb-3 text-sm text-muted-foreground">
        density <Mono>{tokens.density}</Mono> · base unit <Mono>{base}</Mono> — every spacing
        utility is <Mono>calc({base} × n)</Mono>
      </p>
      <div data-hx-token="density" className="space-y-1.5">
        {spacingSteps.map((n) => (
    ...
    <Section title={title}>
      <div className="flex flex-wrap gap-6">
        {entries.map(([key, value]) => (
          <div key={key} data-hx-token={`${tokenGroup}.${key}`} className="text-center">
            {render(key, value)}
            <p className="mt-1">
