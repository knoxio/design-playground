import { icons, type LucideIcon } from "@helix/ui/icons";
  ...
  return (
    <div className="mx-auto max-w-5xl p-8">
      ...
      <p className="mb-6 text-muted-foreground">
        ...
        <span className="font-mono text-sm">@helix/ui/icons</span> — click one to copy its import.
      </p>
      <input
        value={search}
          ...
          {shown.slice(0, 400).map(([name, Icon]) => {
            return (
              <button
                key={name}
                type="button"
                title={`import { ${name} } from "@helix/ui/icons";`}
                onClick={() => {
                  void navigator.clipboard.writeText(`import { ${name} } from "@helix/ui/icons";`);
                  setCopied(name);
                  if (timer.current) clearTimeout(timer.current);
...
/** The icon catalog outside any client context, under the Helix standard. */
