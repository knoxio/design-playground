...
import { DESIGN_STANDARD_THEME } from "../registry/types";
import { resolveTheme } from "./theme";

/**
 * `resolveTheme` picks the active theme by precedence
 * (valid override > experiment base > client default > Design standard) and
...
describe("resolveTheme", () => {
  ...
  it("falls back to the Design standard theme when the default resolves to nothing", () => {
    const client = makeClient({ id: "acme", defaultTheme: "ghost", themes: [] });
    const { currentThemeKey } = resolveTheme({ client, isGallery: false });
    expect(currentThemeKey).toBe(DESIGN_STANDARD_THEME);
  });
