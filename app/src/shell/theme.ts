import { helixTokens, type TokenSet } from "@helix/ui";
import { globalThemes } from "../registry/clients";
import { HELIX_STANDARD_THEME, type ClientEntry, type ThemeEntry } from "../registry/types";
...
export function resolveTheme({
  ...
  const currentThemeKey = validOverride ?? experimentBaseKey ?? defaultKey ?? HELIX_STANDARD_THEME;
  const tokens = themeOptions.find((t) => t.key === currentThemeKey)?.tokens ?? helixTokens;
