...
export function FrameShell() {
  ...
  useEffect(() => {
    post({ hx: "route", route: canonicalRoute });
  }, [canonicalRoute]);
  ...
  useEffect(() => {
    const onMessage = (e: MessageEvent) => {
      ...
      if (data.hx === "comments-active") setCommentsActive(data.active);
    };
  ...
  return (
    <div>
      ...
      <CommentsOverlay
        ...
        onOpenCount={(count) => post({ hx: "open-count", count })}
        onExit={() => post({ hx: "exit-comments" })}
      />
    </div>
  );
}
