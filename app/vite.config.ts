...
import { hxSource } from "./hx-source-plugin";
...
function feedbackProxy(mode: string): Record<string, ProxyOptions> | undefined {
  ...
  return {
    "/api": {
      target: (env.HX_FEEDBACK_URL ?? "https://hx-playground.pages.dev/api").replace(
        /\/api\/?$/,
        "",
      ),
      changeOrigin: true,
...
function watchRegistrySources(): Plugin {
  return {
    name: "hx-watch-registry-sources",
...
export default defineConfig(({ mode }) => ({
  plugins: [
    ...(scopedClient ? [scopeRegistryGlobs(scopedClient)] : [hxSource()]),
